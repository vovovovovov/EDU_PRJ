...
class MLP(nn.Module):
    def __init__(self, gat_out_features, num_classes):
        super(MLP, self).__init__()
        self.linear = nn.Linear(num_classes, gat_out_features)

    def forward(self, gat_output):
        return self.linear(gat_output)
...
class RobertaGAT(nn.Module):
    def __init__(self, roberta_model_name, num_classes):
        ...
        # 加载GAT模型
        self.gat = GATConvWithAttention(self.roberta.config.hidden_size, 128).to('cuda:1')

        self.linear = MLP(num_classes, self.gat.out_channels).to("cuda:1")
    ...
    def forward(self, input_ids, attention_mask, edge_index):
        ...
        return self.linear(gat_output), attention_weights
