...
const applyFilters = () => {
  ...
  if (activeFilter.value !== 'All') {
    tempCards = tempCards.filter(
      (card) => card.type === activeFilter.value
    );
  }
...
<style scoped>
  ...
  section {
    ...
    flex-direction: column;    /* 子元素从上到下排列（由列的方式进行排列）*/
    margin: 0rem 10rem 0rem 10rem;   /* 上下0rem 和 左右00rem */
  }
  ...
  .card .image {
    flex: 0 0 50%; /* 图片区域占卡片的 50% 高度 */
    width: 100%;
  }
  .card .image > img {
    width: 100%;
    height: 100%;
    object-fit: cover; /* 确保图片按照比例填充区域 */
  }
  .card > h2{
    margin: 5px;
  }
